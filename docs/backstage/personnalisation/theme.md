...
| System                   | Package                    | Role                                  | Used by                                      |
| ------------------------ | -------------------------- | ------------------------------------- | -------------------------------------------- |
| **MUI v4** (Material UI) | `@material-ui/core` 4.12.4 | Ancien système, encore majoritaire    | La plupart des plugins, et notre propre code |
| **MUI v5**               | `@mui/material` 5.18.0     | Version plus récente de MUI           | Certains plugins (`plugin-catalog-react`)    |
| **BUI** (Backstage UI)   | `@backstage/ui` 0.18.0     | Nouveau système, adopté petit à petit | Catalogue, templates (scaffolder), TechDocs  |

MUI v4 dessine encore la majorité de l'interface: barre latérale, la plupart des pages, boutons, tableaux. Notre propre code l'utilise aussi (`LogoFull.tsx`, `LogoIcon.tsx`, dépendance déclarée dans `packages/app/package.json`).
...
| File        | System       | Content                                                 |
| ----------- | ------------ | ------------------------------------------------------- |
| `theme.ts`  | MUI v4 et v5 | Thème créé avec `createUnifiedTheme`                    |
| `theme.css` | BUI          | Variables CSS `--bui-*`, fichier importé dans `App.tsx` |

`createUnifiedTheme` vient du paquet `@backstage/theme`. Le thème est déclaré dans l'app avec l'extension `ThemeBlueprint` (paquet `@backstage/plugin-app-react`).
...
- une classe par composant, qui commence par `bui-` suivie de son nom: `bui-Button`, `bui-MenuItem`, `bui-Select`, `bui-SearchField`, `bui-SearchAutocomplete`, `bui-Table`, `bui-Tabs`, `bui-Card`, `bui-TextField`... Les variantes d'un composant se ciblent par ses attributs `data-*` (par exemple `data-disabled`).
...
=== "MUI"
    ...
    Dans `theme.ts`, section `components`:
...
=== "BUI"
    ...
    Dans `theme.css`. Le bouton BUI utilise `--bui-radius-2` pour son arrondi et `--bui-bg-solid` / `--bui-bg-solid-hover` pour sa couleur (vérifié dans `node_modules/@backstage/ui/dist/components/Button/`). Ces variables servent aussi à d'autres composants: pour ne toucher qu'aux boutons, on cible la classe `.bui-Button`:
